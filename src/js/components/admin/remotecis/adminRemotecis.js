...
function AdminRemotecisCtrl($state, $uibModal, api, messages) {
  ...
  $ctrl.deleteRemoteci = function(remoteci) {
    ...
    deleteRemoteciModal.result.then(function() {
      api.remotecis.remove(remoteci.id, remoteci.etag).then(function() {
          messages.alert('remoteci ' + remoteciName + ' has been removed', 'success');
          $state.reload();
        },
        function() {
          messages.alert('remoteci ' + remoteciName + ' can\'t be removed', 'danger');
        });
    });
  }
}
