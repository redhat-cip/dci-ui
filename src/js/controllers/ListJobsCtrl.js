...
require("app").controller("ListJobsCtrl", [
  ...
  function($state, $scope, api, status, appCache) {
    ...
    if (!$scope.remotecis) {
      api.remotecis.list(null, true).then(function(remotecis) {
    ...
    if (statuses.length || remotes.length) {
      api.jobs.search(remotes, statuses).then(function(data) {
      ...
      api.jobs.list(page).then(pagination).then(function(data) {
        $scope.jobs = data.jobs;
        _.each(data.jobs, function(job) {
          api.jobs.results(job.id).then(function(results) {
            _.each(job.jobdefinition.tests, function(test_jobdef) {
