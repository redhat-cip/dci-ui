...
.controller('InformationCtrl', ['$scope', 'api', function($scope, api) {
  ...
  api.remotecis.list(null, true).then(_.partial(_.set, $scope, "remotecis")).then( function() {
    _.each($scope.remotecis, function(remoteci) {
      api.jobs.lastperremoteci(remoteci.id).then(function(job) { remoteci.updated_at = job.updated_at});
    });
  });
