  ...
  '$injector', '$scope', function($injector, $scope) {
    ...
    .then(function(data) {
      ...
      _.each(data.jobdefinitions, function(jobdef) {
        jobdef.created_at_formatted = moment(jobdef.created_at).local().format();
        jobdef.updated_at_formatted = moment(jobdef.updated_at).local().format();
      });
      $scope.pagination = {
        total: data._meta.count, page: page,
        pageChanged: function() {
    ...
    api.topics.list(page)
      .then(function(data) {
        ...
        _.each(data.topics, function(topic) {
          topic.created_at_formatted = moment(topic.created_at).local().format();
          topic.updated_at_formatted = moment(topic.updated_at).local().format();
        });
        $scope.pagination = {
