...
.controller('GpanelStatusCtrl', [
  ...
  function($injector, $scope, topic, componentType) {
    ...
    api.jobdefinitions.list(null, true).then(_.partial(_.set, $scope, 'jobdefs'));
      ...
      _.each($scope.components, function(component) {
        api.topics.components.jobs(topic, component.id).then(function(jobs) {
          _.each(jobs, function(job) {
            _.update(job, 'created_at', _.partial(moment));
            _.update(job, 'modified_at', _.partial(moment));
            job.css = status[job.status];
          });
          _.set($scope, 'jobs.' + component.id + '.' + jobs[0].jobdefinition_id,
                _.sortBy(jobs, function(o) {return o.created_at;}));
        });
      });
    });
  }
