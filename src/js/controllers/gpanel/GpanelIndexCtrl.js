...
require("app").controller("GpanelIndexCtrl", [
  "$scope",
  "api",
  function($scope, api) {
    api.topics.list(null, true).then(function(topics) {
      ...
      _.each(topics, function(topic) {
        api.topics.jobdefinitions(topic.id).then(function(jobDefinition) {
          topic.jobdefinition_type =
            encodeURI(jobDefinition[0]["component_types"][0]) || "puddle_osp";
          api.topics.status(topic).then(function(jobs) {
            topic.jobs = jobs;
            _.each(jobs, function(job) {
              if (
                ["new", "pre-run", "post-run"].indexOf(job.job_status) !== -1
              ) {
                job.state = "text-info";
              } else if (
                ["failure", "product-failure", "deployment-failure"].indexOf(
                  job.job_status
                ) !== -1
              ) {
