...
test("creates SET_USERS when fetching users has been done", t => {
  nock("https://api.example.org/api/v1")
    .get("/users")
    .reply(200, { users: [{ id: "u1" }] });
  const expectedActions = [
    { type: constants("users").SET_LIST, payload: [{ id: "u1" }] }
  ];
...
test("creates user clean object", t => {
  nock("https://api.example.org/api/v1")
...
test("creates DELETE_USER when a user is deleted", t => {
  nock("https://api.example.org/api/v1")
    ...
    .reply(204);
  const expectedActions = [
    { type: constants("user").DELETE, payload: { id: "u1", etag: "e1" } }
  ];
  const store = mockStore({ config: { apiURL: "https://api.example.org" } });
  return store
