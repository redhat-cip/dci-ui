...
test("test create topic form submit the correct values", async () => {
  ...
  } = render(<CreateTopicModal onSubmit={mockOnSubmit} products={products} />);
  ...
  const select = getByPlaceholderText("Select a product") as HTMLSelectElement;
  ...
  await waitFor(() => {
    expect(select.value).toBe(products[1].name);
  });
